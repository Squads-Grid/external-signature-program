...
pub fn verify_and_parse_public_key(&self) -> Result<[u8; 33], &'static str> {
    ...
    for i in cbor_start..self.auth_data.len() - 3 {
        if self.auth_data[i] == 0x21 && self.auth_data[i+1] == 0x58 && self.auth_data[i+2] == 0x20 {
            x_start = i + 3;
            break;
        }
    }
    ...
    let mut y_start = 0;
    for i in cbor_start..self.auth_data.len() - 3 {
        if self.auth_data[i] == 0x22 && self.auth_data[i+1] == 0x58 && self.auth_data[i+2] == 0x20 {
            y_start = i + 3;
            break;
        }
    }
...
fn test_auth_data_parsing_create() {
    ...
    let expected_compression_byte = if (y_coord[31] & 0x01) == 0 { 0x02 } else { 0x03 };

    // Check if the compression byte matches what we expect based on y's parity
    assert_eq!(public_key[0], expected_compression_byte, "Incorrect compression byte");
}
